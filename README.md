...
## 🙋 고르기 시트 — 아이가 고르고, A4 로 뽑는다 (2026-08-22 신설)

확정 일정은 어른이 정하고, **그 위에 얹을 것은 아이가 고른다.** 결정권을 주면 이동 시간
불평이 줄고, 무엇보다 "가고 싶다고 말한 곳"이라 현장에서 잘 논다.

- 화면에서 **언니 / 동생** 버튼을 각자 누른다. 둘 다 누르면 초록 테두리 + `둘 다 골랐어요` = 합의.
- 일자 카드의 **`🖨️ 이 날만 A4`** → 그 날짜 한 장(실측 A4 1페이지).
  지도 아래 **`🙋 고르기 시트 (A4)`** → 전 일정.
- 화면에서 고른 건 종이에도 ● 로 찍혀 나온다. 종이로만 고를 거면 빈 동그라미를 색칠하면 된다.
- 시트에는 **핵심만** 넣는다 — 그 날 정해진 동선 한 줄 + 고를 것 + 서명란. 비용표·준비물·
  렌터카 안내는 넣지 않는다(그건 `🖨️ 전체 요약 인쇄`가 소유).

### 다른 날짜·다른 도시에 그대로 쓰는 법

`plan-specs.json` 의 **아무 day 에나 `picks` 를 추가하면 끝**이다. 코드는 안 고친다.

```jsonc
"picks": {
  "title": "9/23 (수) — 동쪽 다녀오는 길에 하나만",
  "max": 1,                       // 몇 개까지 고를 수 있는지 (시트에 표기)
  "note": "오늘은 차에서 2시간 30분을 보내는 날이야. 그래서 딱 하나만.",
  "options": [
    { "ref": "sangumburi",        // attractions.json 의 id — 이름·요금·링크를 끌어온다
      "tag": "오름",               // 한 단어 분류 (뱃지)
      "dur": "1시간",              // 소요시간
      "cost": "소인 4,000원",       // 아이가 이해할 표기
      "move": "섭지코지에서 차 40분 [추정]",
      "why": "오름인데 길이 평평해서 안 힘들어. 큰 분화구를 위에서 내려다봐." },

    { "id": "pool-1",             // 장소가 아닌 활동은 ref 대신 id + name
      "name": "호텔 실내수영장",
      "tag": "수영", "dur": "1시간", "cost": "투숙객 무료",
      "why": "밖이 추워도 여긴 따뜻해." }
  ]
}
```
...
```bash
python3 scripts/build_plans.py data/jeju   # picks 는 그대로 통과된다
python3 scripts/validate.py                # ref 오타·id 중복·max 범위를 코드가 막는다
```

`validate.py` 가 잡아 주는 것: 존재하지 않는 `ref`(화면에서 빈 줄로만 보여 눈으로 못 잡는다),
`id` 중복(선택 기록이 섞인다), `max` 가 후보 수보다 큰 경우. `why`·`dur` 누락은 경고.

**아이가 읽는 문장**이라 `why` 는 반말·짧게 쓴다. 요금은 지어내지 말고, 확인 못 한 값은
`[추정]` / `[확인필요]` 를 그대로 남긴다.
